...
const initializers = {
  ...
  dfd: async () => {
    const { createDfdServices } = await import('./language/dfd/index.js');
    const parser = createDfdServices().Dfd.parser.LangiumParser;
    parsers.dfd = parser;
  },
} as const;
...
export async function parse(diagramType: 'dfd', text: string): Promise<Dfd>;
